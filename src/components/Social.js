// esg-frontend/src/components/Social.js

import React, { useState, useEffect, useCallback } from 'react';
import { obterDadosSocial, obterPerguntasGeneraisSocial, salvarDadosSocial, atualizarDadosSocial, deletarDadosSocial } from '../services/Api';
...
const Social = () => {
  ...
  const carregarDadosIniciais = useCallback(async () => {
    try {
      const dadosSocial = await obterDadosSocial(usuario.empresa_id);
      if (dadosSocial && dadosSocial.length > 0) {
        setDados(dadosSocial.map(item => ({ ...item, id: String(item.id) })));
        setEditando(false);
        setDadosSalvos(true);
      } else {
  ...
  const adicionarNovosDados = async () => {
    const perguntasGenerais = await obterPerguntasGeneraisSocial();
  ...
  const salvarDados = async () => {
    ...
    try {
      await Promise.all(dados.map(item => item.id.startsWith('geral-')
        ? salvarDadosSocial({ ...item, id: undefined })
        : atualizarDadosSocial(item.id, item)));
  ...
  const deletarTodosDados = async () => {
    ...
    try {
      await deletarDadosSocial(usuario.empresa_id);
