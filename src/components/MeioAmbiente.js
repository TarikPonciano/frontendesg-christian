// esg-frontend/src/components/MeioAmbiente.js

import React, { useState, useEffect, useCallback } from 'react';
import { obterDadosAmbientais, obterPerguntasMeioAmbiente, salvarDadosAmbientais, atualizarDadosAmbientais, deletarDadosAmbientais } from '../services/Api';
...
const MeioAmbiente = () => {
    ...
    const carregarDados = useCallback(async () => {
        try {
            const dadosAmbientais = await obterDadosAmbientais(usuario.empresa_id);
            if (dadosAmbientais && dadosAmbientais.length > 0) {
                setDados(dadosAmbientais.map(item => ({
                    ...item,
                    id: String(item.id)
                })));
                setEditando(false);
                ...
                console.log('Nenhum dado encontrado para carregar.');
            }
        } catch (erro) {
    ...
    const adicionarNovosDados = async () => {
        const perguntasGerais = await obterPerguntasMeioAmbiente();
        const novosDados = perguntasGerais.map(pg => ({
            ...pg,
            id: `geral-${pg.id}`,
            avaliacao: '',
            oquefazer: '',
    ...
    const salvarDados = async () => {
        ...
        try {
            await Promise.all(dados.map(item => 
                item.id.startsWith('geral-') 
                ? salvarDadosAmbientais({...item, id: undefined, empresa_id: usuario.empresa_id}) // Adiciona empresa_id aqui
                : atualizarDadosAmbientais(item.id, {...item, empresa_id: usuario.empresa_id}) // Adiciona empresa_id aqui
            ));
    ...
    const deletarTodosDados = async () => {
        ...
        try {
            await deletarDadosAmbientais(usuario.empresa_id);
